...
public class AutoDetectParser extends CompositeParser {
    ...
    public void parse(
        ...
        try {
            ...
            //time:2018.08.08
            //auther:liu
            //将meta中的密码写入context，Tika中解密所用验证密码大多数是从里面获取
            final String passwd = metadata.get("password");
            if(passwd != null)
            {
                if(context.get(PasswordProvider.class) == null) {
            ...
            //time:2018.08.14
            //auther:liu
            //取出context中的PasswordProvider
            if(context.get(PasswordProvider.class) != null) {
                context.set(PasswordProvider.class, null);
            }
            //end
            tmp.dispose();
        }
